...
<div class="bx_login_block  pull-right">
	...
	$frame = $this->createFrame("login-line", false)->begin();
		...
		{
		?>
			<a class="bx_login_top_inline_link  bxr-color-button" href="javascript:void(0)<?//=$arResult["AUTH_URL"]?>" onclick="openAuthorizePopup()">
				<span class="fa fa-sign-in"></span>
				<?=GetMessage("AUTH_LOGIN")?>
			</a>
			<?if($arResult["NEW_USER_REGISTRATION"] == "Y"):?>
			<a class="bx_login_top_inline_link  bxr-color-button" href="<?=$arResult["AUTH_REGISTER_URL"]?>" >
		...
		else
		{
			...
			if (strlen($name) <= 0)
				$name = $USER->GetLogin();
		?>
			<a class="bx_login_top_inline_link  bxr-color-button" href="<?=$arResult['PROFILE_URL']?>">
				<span class="fa fa-user"></span>
				<?=htmlspecialcharsEx($name);?></a>
			<a class="bx_login_top_inline_link  bxr-color-button" href="<?=$APPLICATION->GetCurPageParam("logout=yes", Array("logout"))?>">
				<span class="fa fa-sign-out"></span>
				<?=GetMessage("AUTH_LOGOUT")?></a>
		<?
		}
	$frame->beginStub();
		?>
