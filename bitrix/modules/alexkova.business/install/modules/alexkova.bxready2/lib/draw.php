...
class Draw extends \Alexkova\Bxready2\Allbxready {
	...
	public function showElement($elementLib, $elementType, $arElement = array(), $arElementParams = array(), $onlyEpilog = false){

		$this->clearAdditionalFiles();
		$this->setUseExtFiles(false);

		return self::drawElement($elementLib, $elementType, $arElement, $arElementParams, $onlyEpilog);
        }
	...
	private function drawElement($elementLib, $elementType, $arElement = array(), $arElementParams = array(), $onlyEpilog = false){
		...
		/*if ($arElementParams["BXREADY_USER_TYPES"] == "Y"){

			if (strlen($arElementParams["BXREADY_USER_TYPE_VARIANT"]) > 0){
				$elementType = $arElementParams["BXREADY_USER_TYPE_VARIANT"];
			}

			$pathPrefix = SITE_TEMPLATE_PATH.'/bxready2/collection/user/'.$elementType;
			...
			$pathPrefix = SITE_TEMPLATE_PATH.'/bxready2/collection/bxr_'.$elementLib.'/'.$elementType;

			if (!file_exists($_SERVER["DOCUMENT_ROOT"].$pathPrefix."/element.php")){
				$pathPrefix = '/bitrix/templates/.default/bxready2/collection/bxr_'.$elementLib.'/'.$elementType;
				if (file_exists($_SERVER["DOCUMENT_ROOT"].$pathPrefix."/element.php")){
					$getElement = true;
				}
			}else{
