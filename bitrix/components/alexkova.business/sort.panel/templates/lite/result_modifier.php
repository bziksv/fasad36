...
if (empty($arResult["SORT_PROPS"])) {
    $arResult["SORT_PROPS"] = array(
            "PROPERTY_BXR_INSTOCK_VALUE" => array("PROPERTY_BXR_INSTOCK_VALUE", "asc", "По наличию"),
            "NAME" => array("NAME", "asc", GetMessage("BXR_SORT_NAME_NAME")),
            /*"PROPERTY_RATING" => array("PROPERTY_RATING", "asc", GetMessage("BXR_SORT_RATING_NAME")),*/
            "SORT" => array("SORT", "asc", GetMessage("BXR_SORT_SORT_NAME")),
            "PROPERTY_BXR_PRICE" => array("PROPERTY_BXR_PRICE", "asc", GetMessage("BXR_SORT_PRICE_NAME")),
    );
}
...
function NumberWordEndingsEx($num, $arEnds = false) {
   $lang = LANGUAGE_ID;
   if ($arEnds===false) {
      if(strlen(GetMessage("BXR_ITEM_NAME_ENDING")) > 0):
	$arEnds = explode(",", GetMessage("BXR_ITEM_NAME_ENDING"));
      endif;
   }
   if ($lang=='ru') {
...
$sort_order = array_key_exists("order", $_REQUEST) && in_array(ToLower($_REQUEST["order"]), Array("asc", "desc")) ? ToLower($_REQUEST["order"]) : $arResult["SORT_PROPS"][$sort][1];
