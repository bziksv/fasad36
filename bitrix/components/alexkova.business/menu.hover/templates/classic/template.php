...
<?
    if(!function_exists(bxr_classic_build_tree)) {
        function bxr_classic_build_tree($mas, $ico = "N", $arColor = array("li" => "", "li_selected" => "", "ico_1" => "color", "ico_2" => "light"), $arParams = [], $lvl=1) { 
            ...
            foreach($mas as $k => $v) {
                ...
                if(isset($v['CHILDREN'])){
					...
					$link = "<a class='sub-item' href='" . $v['LINK'] . "'><i class='fa fa-angle-left'></i><span class='bxr-ico-hover-menu'>". $s_ico . $s_ico_h . "</span>" . $v['TEXT'] . "<i class='fa fa-angle-right'></i></a>";
					
					if (in_array($v['PARAMS']['ID'], getIndexes())){
						$js = '<i class="fa fa-angle-left"></i><span class="bxr-ico-hover-menu">'. $s_ico . $s_ico_h . '</span>' . $v['TEXT'] . '<i class="fa fa-angle-right"></i>';
						$link = "<a class='sub-item' href='" . $v['LINK'] . "' text-js='$js'></a>";
					}
					
                    $s_result .= "<li ".$isSelected." class='" . $li_class . "' >" . $link . bxr_classic_build_tree($v['CHILDREN'], $ico, $arColor, ($lvl+1)).
                                "</li>";
                }else{
					$link = "<a ".$li_a_class."  href='" . $v['LINK'] . "' data-id='" .$v['PARAMS']['ID']."' >".$fa_left_o."</i><span class='bxr-ico-hover-menu'>". $s_ico . $s_ico_h . "</span>" . $v['TEXT'] . "</a>";
					
					if (in_array($v['PARAMS']['ID'], getIndexes())){
						$link = "<a ".$li_a_class."  href='" . $v['LINK'] . "' data-id='" .$v['PARAMS']['ID']."'  text-js='" . $v['TEXT'] . "'>".$fa_left_o."</i><span class='bxr-ico-hover-menu'>". $s_ico . $s_ico_h . "</span></a>";
					}
					
                    $s_result .= "<li ".$isSelected." class='" . $li_class. "' >". $link ."</li>";
                }
            }
...
<div class="bxr-classic-hover-menu <?if($arParams['COLOR_MENU'] == "light") {echo " menu-arrow-top";} ?><?if($arParams['STYLE_MENU_HOVER'] == "colored_color") {echo " bxr-classic-hover-menu-color";} ?><?if($arParams['STYLE_MENU_HOVER'] == "colored_dark") {echo " bxr-classic-hover-menu-dark";} ?>">
    <?  
        ...
        $tree = bxr_classic_build_tree($arParams['MENU_TREE'], $isIco, $arColorParams, $arParams, 1);       
        echo $tree;
    ?>
</div>