...
<div class="col-xs-12 request-product">
    <?if ($arResult["ELEMENT"]["PICTURE"]) {?>
        <div class="request-product-img">
            <img src="<?=$arResult["ELEMENT"]["PICTURE"]?>">
        </div>
    <?}?>
...
<?if (strlen($arResult["MESSAGE"]) > 0):?>
	...
	<?return false;?>
<?endif?>
<form id="iblockForm<?=$arParams["IBLOCK_ID"]?>_<?=$BXR_FORM_COUNTER?>" class='bxr-form-body' name="iblock_add" action="<?=$arParams["POST_FORM_URI"]?>" method="post" enctype="multipart/form-data">
	...
	<?if ($arParams["MAX_FILE_SIZE"] > 0):?><input type="hidden" name="MAX_FILE_SIZE" value="<?=$arParams["MAX_FILE_SIZE"]?>" /><?endif?>
		<?if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])):?>
			<?foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
                                        ...
                                        <?if ($inputType != 'hidden') {?>
                                            <?if (intval($propertyID) > 0 and $arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] != 'RULE'):?>
											...
											<?endif?>
											<?if(in_array($propertyID, $arResult["PROPERTY_REQUIRED"]) and $arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] != 'RULE'):?>
	...
	<button onclick="BXReady.Business.formRefresh('<?=$arParams["IBLOCK_ID"]?>_<?=$BXR_FORM_COUNTER;?>');return false;" class="bxr-color bxr-color-button" id="submitForm_<?=$arParams["IBLOCK_ID"]?>">
...
<script>
    $(function() {
       $('textarea[data-code="USER_COMMENT_AREA"]').html('<?=$arResult["ELEMENT"]["MESSAGE"]?>'); 
       $('input[data-code="TRADE_ID_HIDDEN"]').val('<?=$arResult["ELEMENT"]["ID"]?>'); 
       $('input[data-code="TRADE_NAME_HIDDEN"]').val('<?=$arResult["ELEMENT"]["NAME"]?>'); 
       $('input[data-code="TRADE_LINK_HIDDEN"]').val('<?=$arResult["ELEMENT"]["LINK"]?>'); 
       if (parseInt('<?=$arResult["ELEMENT"]["OFFER_ID"]?>') > 0)
           $('input[data-code="OFFER_ID_HIDDEN"]').val('<?=$arResult["ELEMENT"]["OFFER_ID"]?>'); 
    });
</script>